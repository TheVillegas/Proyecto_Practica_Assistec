...
ReporteRAM.crearReporteRAMInicial = async (codigoALI, client = null) => {
    try {
        ...
        logger.error(`[ReporteRAM] Error creando reporte inicial para ALI ${codigoALI}`, { message: err.message });
        throw err;
    }
};
...
ReporteRAM.obtenerEstadoRAM = async (codigoALI) => {
    try {
        ...
        logger.error('Error obteniendo estado RAM', { message: err.message });
        throw err;
    }
};
...
ReporteRAM.obtenerReporteRAM = async (codigoALI) => {
    ...
    try {
        ...
        logger.error('Error obteniendo Reporte RAM', { message: err.message });
        throw err;
    } finally {
...
ReporteRAM.guardarReporteRAM = async (datos, rutUsuario) => {
    ...
    try {
        ...
        logger.error('Error guardando RAM', { message: err.message });
        throw err;
    } finally {
        if (client) client.release();
    }
...
ReporteRAM.procesarEtapa3RAM = async (datos) => {
    ...
    try {
        ...
        if (client) await client.query('ROLLBACK');
        logger.error('Error etapa 3 RAM', { message: err.message });
        throw err;
    } finally {
        if (client) client.release();
    }
...
ReporteRAM.calcularRecuentoColonias = (datos) => {
    try {
        ...
        logger.error('Error al calcular recuento de colonias', { message: error.message });
        return { ufc: null, textoReporte: "Error en cálculo", operador: "=", esEstimado: false, casoAplicado: "ERROR" };
    }
};
