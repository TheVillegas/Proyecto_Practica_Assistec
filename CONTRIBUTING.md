...
`main` está protegido: **solo se mergea via Pull Request** con CI verde y aprobación del admin.

Si necesitás ayuda o algo no funciona, hablá con el admin del repositorio.

---

## Flujo de trabajo (lo más importante)
...
1. **Crear una rama** desde `main` con el formato adecuado (ver [convenciones de ramas](#convenciones-de-ramas)).
2. **Desarrollar y commitear** siguiendo [Conventional Commits](#convenciones-de-commits).
3. **Ejecutar tests localmente** antes de abrir el PR (ver [tests](#tests)).
4. **Abrir un Pull Request** en GitHub con descripción clara de los cambios y su motivo.
5. **Esperar el CI** — se ejecuta automáticamente. Si falla, corregir.
6. **El admin revisa** y si todo está bien, aprueba y mergea.
